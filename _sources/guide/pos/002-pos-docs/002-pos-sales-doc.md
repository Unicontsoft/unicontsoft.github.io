...
Списъкът съдържа колони с основни данни за настроените продукти - код, наименование, мерна единица, цена, баркод и други. Чрез полето **Търсене** могат да бъдат намерени продукти по код, име, баркод.  

В списъка се визуализира и колона за група. Полетата в нея се обзавеждат за всеки продукт с настроената през **Dreem ERP** дименсия, в която участва. Списъкът може да бъде разделен по групи чрез маркиране на опцията **Групиране**. Тя се намира непосредствено след полето за търсене.    

Бутон **Избор** добавя маркирания продукт в текущата продажба.  
Бутон **Отказ** затваря формата **Избор на продукт** и връща на основен екран и текущата продажба.  

## **Промяна на количество**

> По подразбиране системата добавя нов продукт с количество **1** (едно). То може да бъде променено по всяко време преди приключване на продажбата.  

- **На новия ред**  
Когато продуктите се добавят чрез сканиране или избиране от меню, количество се въвежда предварително. Желаната стойност се попълва на новия ред - самостоятелно или като се отдели със символ звезда [ * ] преди/след количеството.     
При добавяне на продукти чрез код/име количеството се отделя със символ звезда: 5[*]0123. Изборът се потвърждава с клавиш [**+**] или [**Enter**]. 

- **На предходни редове**  
За навигиране до предходен продукт се използват стрелките нагоре/надолу или мишката.  
Количеството на реда се увеличава с клавиш [**+**] или се намалява с клавиш [**-**]. Промяна може да бъде направена също с изписване на символ звезда и ново количество: [*]5. Друг вариант е количеството да бъде модифицирано чрез редакция на ред с [**F4**].  
...
В лявата част на форма **Плащане** са разположени полета, които се попълват със сумите в евро. Това включва настроените типове плащания, тотали за крайна сума, платено и ресто в евро.  

В средната част на формата е добавено поле за получена от клиент сума в лева. Системата изчислява левова равностойност на крайната сума, на общо получената сума и на рестото в лева. Информацията е показана в полета с тотали в средната част на форма **Плащане**.  
