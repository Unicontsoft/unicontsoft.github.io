...
- [Разпределение на разходи при общ документ с покупката на стоки](#стоки-и-разходи-за-придобиване-с-общ-документ)    
- [Справки за разпределение на разходите по придобиване](#справки)  
- [Счетоводни настройки и документи](#счетоводни-данни)  

## **Въведение**

Получените от доставчик стоково-материални запаси се отразяват в системата чрез [въвеждането на документи за покупка](../002-docs/002-trade-system/001-orders-sales-purchase-documents/002-create-purchase-documents.md). Към тях се генерират складови документи, с което продуктите се заприходяват по покупна цена(без ДДС).  
...
Схемата е описана и в статията [Разходи за придобиване](../002-docs/002-trade-system/001-orders-sales-purchase-documents/009-purchases-transport-expenses.md), с която е добре да се запознаете, ако темата е нова за Вас.   
...
## **Стоки и разходи за придобиване с общ документ**

Има случаи, в които доставчикът издава фактура, включваща както стоката, така и допълнителни услуги. Например транспортни разходи, когато доставчикът транспортира стоката.  

Отново разпределяте разхода по описания начин с една особеност - документът за покупка трябва да е валидиран с приключен ПСД към него. След това покупката се връща в състояние на редакция.  

![](904-allocate-acquisition-costs10.png){ class=align-center w=15cm }

Отваряте меню **Средства » Разпределение на разходи за придобиване** и следвате стъпките.
...
Продължавате напред и избирате желания метод на разпределението на разхода за транспорт. Завършвате процеса по гореописания начин.
