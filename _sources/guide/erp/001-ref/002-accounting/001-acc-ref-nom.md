...
Тези параметри са достъпни за настройка в **Референтни номенклатури** и са разпределени в три секции: *Счетоводство*, *Интрастат* и *Дълготрайни активи*.   

## **Нова референтна номенклатура**

Процесът по добавяне на нова референтна номенклатура е следният:  

1) Избира се група функции [**Номенклатури » Референтни номенклатури**](../001-nomenclatures/001-ref-nomenclatures.md).  
Нова референтна номенклатура се добавя чрез реда за нов запис, стоящ в началото на всеки списък.   

![](901-acc-ref-nom1.png){ class=align-center w=15cm }
...
3) **Колони на дневниците** - За добавяне на настройка за нова колона в дневниците по ДДС се попълват полета: **Код** - с номер на колоната на реда; **Дневник** - с избор на вид дневник по ДДС, **Колони на дневниците** - с наименование на колоната за съответния дневник.   
...
7) **Направления** - За добавяне на настройка задължително се попълва поле **Направления** с наименование. Поле **Код** се обзавежда по желание.   
...
12) **Групи ДА** - Номенклатурата включва списък с групите дълготрайни активи. Групите са системно добавени и могат да бъдат редактирани.  
...
14) **Коефициент на преоценка** - Номенклатура се добавя с попълване на поле **Година**, за която ще се отнася процентът на преоценка.  
...
- **Допълнителен код** - полето се обзавежда с допълнителен код (превод) чрез десен клик на реда и *Преведи*;   
- **Дневници по ДДС** - наименование на дневника по ДДС;  
- **Допълнително име** - полето се обзавежда с допълнително наименование (превод на различни езици) чрез десен клик на реда и *Преведи*;   
...
- **Дн. за покупки колона** - отваря падащ списък за избор на колона от дневник за покупки;  
Реквизитът се попълва, когато основанието на реда е свързано с покупки.  
- **Дн. за продажби колона** - отваря падащ списък за избор на колона от дневник за продажби;  
Реквизитът се попълва, когато основанието на реда е свързано с продажби.  
...
- **Условия на доставка** - наименование за вид на доставката;  
- **Забележка** - кратко уточнение към условията на доставка;  
- **Термин** - указва международен търговски термин за вида на доставката;  
...
- **Година** - поле с година, за която се отнася процент на преоценка;  
