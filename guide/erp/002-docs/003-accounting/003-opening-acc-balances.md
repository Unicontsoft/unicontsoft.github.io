...
**Dreem ERP** разполага с инструмент за автоматична генерация на начални салда на счетоводни сметки. Чрез него за избрана година могат да бъдат създадени записи накуп за всички сметки или поетапно за една/няколко сметки.  
...
В поле **Сметка** се отваря списък с главни сметки от настроения **Сметкоплан**. Възможните варианти за избор са:  
...
От генерацията могат да бъдат изключени една или няколко сметки. За целта се използва списъкът в поле **Без сметки**, който позволява множествен избор.  
Ако полето е празно, системата изпълнява генерацията спрямо критериите в полета **За година** и **Сметка**.   

![](903-opening-acc-balances3.png){ class=align-center }

Избраните критерии се потвърждават с бутон [**Напред**]. Системата прави нужните проверки и показва резултата от генерацията.  

При съобщение за грешка нередностите трябва да бъдат отстранени. След това може да се направи повторен опит за генерация.  

Когато НС са успешно генерирани, формата се затваря от [**X**].  

![](903-opening-acc-balances4.png){ class=align-center }

Системата създава отделен документ с НС за всяка от главните сметки. Записите са в редакция и могат да бъдат проверявани, анулирани и приключвани поетапно.  
...
Избрани данни за НС на сметки могат да бъдат коригирани от формата за редакция. Това се отнася до оцветените в жълто полета.  

> Системата приема за валидни единствено потвърдените с **Приключен** начални салда.  
...
1) В раздел **Документ**:  
   ...
   - **Плащания/прихващания** - полето се обзавежда при свързване на плащания  платежни документи (ПКО, РКО, банкови извлечения);  
