...
- [Въведение](#въведение)  
- [Съставяне на нова РПВ](#съставяне-на-рпв)  
- [Реквизити](#реквизити)  
...
За автоматичното съставяне на РПВ е необходимо да бъдат извършени предварително следните действия:  
 ...
 - Създаване на номенклатурите със служителите на организацията;  
...
Нова РПВ се създава от модул  **ТРЗ » Разчетно–платежни ведомости**. Чрез десен клик върху списъка със записи се избира **Нов документ**.  
Отваря се нова форма за въвеждане на данни.  

**2. Попълване на реквизити**  

В раздел **Документ** се попълват следните реквизити:  

   - **Док. тип** – От падащия списък в полето се избира тип на документа **РПВ**-*Разчетно–платежна ведомост*.  
   - **Док. No.** – В полето може да се въведе номер за документа. Ако бъде оставено празно, при валидиране на **РПВ** ще се попълни пореден номер.  
   ...
   - **Период** – В полето се указва месец, за който се отнасят заплати на служителите.  
   - **Брой раб. дни** – Реквизитът се попълва автоматично с брой работни дни за избрания период.  
   ...
   Всички официално приети за неработни дни се попълват автоматично при смяна на реализацията на системата.  
   - **Авансова РПВ** – От бутон [**...**] в края на полето се отваря форма за избор на документи **Авансови разчетно–платежни ведомости**. Полето трябва да се обзаведе, когато има създаден документ за аванси за същия месец. При изчисление на заплатите системата ще извади сумите на авансите.  

 ![](902-payroll-documents1.png){ class=align-center w=15cm }

Чрез жълтия бутон [**Създаване редове на РПВ**] в лентата с инструменти може автоматично да се генерира ред за всеки настроен служител. Когато всичко е попълнено правилно, останалите полета от списъка се обзавеждат автоматично.  

> Всички полета в жълт цвят могат да бъдат коригирани.  
 
От раздел **Обща рекапитулация** може да се види общата сума на дължимите осигуровки по параграфи. Справката може да бъде разпечатана при приключване на документа.  

В раздел **Направление** системата показва списък с всички служители, въведени в текущия документ. Те могат да бъдат разпределени като разход към структурни центрове на себестойност, обект/проект и/или финансова структура.  

**3. Приключване**  

Чрез бутон **Приключен** от лентата с инструменти **РПВ** се валидира. Това отваря форма за генерация **Свързани документи**.  
   - **Плащане в** (каса) — Чрез опцията се избира каса, в която се създава разходен касов ордер. Използва се, когато има изплащане на заплати в брой.  
        - *За дата* - Реквизитът указва дата, с която системата попълва **Док. дата** в касовия документ.  
        - *Основание* - От падащия списък се избира основанието за плащане, което се обзавежда в касовия документ.  
   ...
   - **Генериране на Счетоводно записване** - Тази опция указва генериране на свързан счетоводен документ към **РПВ**.  
...
1) В раздел **Документ**:  
   ...
   Списъкът трябва да е настроен предварително в **Номенклатури » Референтни номенклатури**.  
   ...
   - **Дата на постъпване** - обзавежда се спрямо настройката на **Дата на постъпване** за служителя на реда;  
   - **Категория труд** - указва категория труд за служителя на реда;  
   Всички категории трябва да са въведени предварително от **Номенклатури » Референтни номенклатури**.  
   ...
   - **Социални разходи: Общ %** - показва общия процент на настроените за служителя социални разходи;  
   - **Социални разходи: Сума** - показва сума на настроените за служителя социални разходи;  
   ...
   - **Сума за получаване** - указва сума, която се изплаща на служителя;  
...
3) В раздел **Обща рекапитулация**:  
   ...
   - **НПЗ** - използва се в раздел *Разходен център*;  
