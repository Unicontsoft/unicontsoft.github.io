...
- [Въведение](#въведение)
- [Създаване на нов продукт](#добавяне-на-продукт)  
- [Реквизити](#реквизити)  

## **Въведение** 

В номенклатура **Продукти** се включват всички услуги, стоки, продукти, материали и други артикули, с които ще се работи.  
За цялостното функциониране на **Продукти** е необходимо предварително да бъдат настроени свързаните с тях референтни номенклатури. Такива са **Тип**, **Мярка**, **Фасети на мерки**, **Потребителски дименсии** и други.  

## **Добавяне на продукт**  

1. **Създаване**

В меню **Номенклатури » Продукти и материали** с десен клик върху списъка с продукти се избира **Нов продукт**.  
...
Като минимум за създаване на нов продукт системата изисква попълване с валидни данни на полетата за име, тип и мерна единица.   
**Тип на продукта** е референтна номенклатура, свързана пряко с настройките на [**Автоматичен осчетоводител**](../002-accounting/003-acc-wizard.md). Спрямо него системата генерира счетоводните записвания по продукт. Чрез типа на продукта се определя дали артикулът е стока, услуга, ДМА или др.  
В общия случай при настройка на мерна единица се избира най-малката мярка, в която се отчита и се оперира с продукта.  
...
От реда за нов запис могат да се добавят клиентски ценови листи, в които продуктът участва. В последствие от списъка могат да се коригират, изтриват и добавят данни с ценови условия.  

3. **Запис**  

Чрез бутон [**Запис и изход**] в лентата с инструменти настройките се запаметяват и формата за редакция се затваря. С това продуктът е добаявен в списъка.  
...
1) В раздел **Основни**:  
   ...
   - **Активен** - чрез поставяне/махане на отметка продуктът се маркира като активна/неактивна номенклатура;  
   - **Име** – попълва се наименование на продукта;   
   Реквизитът е задължителен.  
   - **Тип** – в полето се отваря падащ списък за избор на тип за продукта;  
   Списъкът трябва да е предварително настроен в **Номенклатури » Референтни номенклатури**.  
   Реквизитът е задължителен.  
   - **Мярка** – указва основна мерна единица, в която системата ще обработва количествата на продукта в складови документи;  
   Реквизитът е задължителен.  
   - **Склад** – поле за избор на склад, който системата третира като склад по подразбиране;  
   - **Група** - отваря списък за избор на основна дименсия, към която продуктът принадлежи;  
   Списъкът трябва да е предварително настроен в [**Номенклатури » Потребителски дименсии**](011-custom-dimensions.md).  
   Името на реквизита варира според наименованието на дефинираната **Дименсия за групи** в меню **Администрация » Настройки » Продукти и материали**.   

   В дясната част може да се показва изображение от настроените в раздел **Списъци: Прикачени файлове**.  

   От реда за добавяне на нов запис се въвежда списък с ценови листи.  
   - **Икона** - полето съдържа изображение със символ, който варира според състоянието на реда/настройката;  
   - **Ценова листа** - отваря се падащ списък за избор на ценова листа, в която текущият продукт участва;  
   Списъкът трябва да е настроен предварително в [**Номенклатури » Ценови листи**](006-price-lists).  
   - **Мярка** - указва мерна единица, за която се настройва цена в ценовата листа на реда;  
   - **Цена** - в полето се въвежда цена без ДДС за мерната единица и ценовата листа на реда;  
   - **Цена с ДДС** - въвежда се цена с ДДС за избраните мярка и ценова листа на реда;  
   - **Валута** - отваря списък за избор на валута за цената по ценова листа;  
   - **Курс** - в полето се попълва курс за избраната валута на реда;  
   - **ДДС вкл. в цената** - указва дали ДДС е включен в цената;  
   - **ДДС ставка** - показва ДДС ставката, настроена за продукта;  
   ...
   - **Активна** - с поставяне/махане на отметка настройката на реда се маркира като активна/неактивна номенклатура;  
   ...
   - **Дата последна модификация** - дата и час на последно направените изменения в данните на текущия ред;  

2) В раздел **Допълнителни**:  
   **Реквизити: Дименсии**  
   - **Група** - В тази секция с отделни полета се визуализират дименсии, отнасящи се за продукти. Използват се за групиране на списък с продукти.   
   Настройват се по желание. Избират се дименсиите (категориите), към които продуктът принадлежи.  
   ...
   **Реквизити: Основни**  
   ...
   - **Продажна мярка** - указва мерна единица, която се обзавежда при избор на продукта в документи за заявка/продажба;    
   - **Допълнителна мярка** - указва допълнителна мярка, в която се визуализират количества и цени;  
   Използва се при печат на складови документи, на ценови листи и на етикети на продукт/материал.  
   - **Ценообразуване при заприходяване в склада** - поле за избор на метод на ценообразуване на компонентите, от които е съставен текущият (комплексният) продукт;  
   ...
   **Реквизити: Счетоводно отчитане**
   - **По признаци** – с настройка *Да*/*Не* указва дали този продукт автоматично се прехвърля в списъка със счетоводни признаци;  
   - **По количество** – с настройка *Да*/*Не* указва дали счетоводното отчитане на продукта е по количество;  

   **Реквизити: Мерки**  
   - **Нето тегло - кг** - попълва се стойност на нето тегло за продукта в килограми;  
   Използва се за ценообразуване при генериране на складов документ от покупка/продажба и при *Интрастат*.  
   Настройката е свързана с реквизит *Ценообразуване при заприходяване в склада*.  
   ...
   Използва се за ценообразуване при генериране на складов документ от покупка/продажба.   
   Настройката е свързана с реквизит *Ценообразуване при заприходяване в склада*.  
   ...
   Използва се в редовете на документи за заявка/продажба.  
   ...
   Използва се в редовете на документ за заявка/продажба.  
   ...
   **Реквизити: Трансформиране при фактуриране**  
   ...
   - **Мярка** - избор на мерна единица на събирателен продукт за трансформация;  
   ...
   Използва се при генерация на счетоводен запис на фактура. Подменя няколко продукта от редовете към един събирателен продукт в счетоводния документ.  
   ...
   **Реквизити: Нормативна цена**  
   ...
   Използва се при избор на продукта в редовете на складов документ и при генерация на **ПСД** от покупка.  
   - **Цена по подразбиране** - избор на цена по подразбиране, която служи за определяне на цена на придобиване на продукта;  
   Използва се при избор на продукта в редовете на складов документ и при генерация на **ПСД** от покупка.  
   ...
   **Реквизити: Отпечатване**  
   ...
   Използва се при печат на етикети за складови локации от функционалност Печат на етикети в меню **Номенклатури » Печат етикети**.  
...
3) В раздел **Списъци**:  
   ...
   **Списъци**
   ...
   - **Складови локации** - указва разположение по складови локации за текущ продукт;  
