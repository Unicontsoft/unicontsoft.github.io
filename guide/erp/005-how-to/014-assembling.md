...
1) **Разходен протокол за разкомплектуване** - От **Търговска система || Складови документи** чрез десен бутон на мишката се избира **Нов документ** (клавишна комбинация Ctrl+N). Отваря се празна форма за въвеждане на нов складов документ.  
...
3) **Генериране през рецепта** - Опцията активира помощник за генерация на свързани документи и е достъпна от меню **Средства**.  
Чрез нея системата автоматично създава приходния складов документ **ППР**. В него автоматично ще се обзаведат всички съставни части на комплекта.  
...
**РПР** се валидира чрез бутон [**Приключен**] и системата предлага автоматична генерация на счетоводно записване. С потвърждаване на избора в документа се визуализират цена и стойност.   
...
Приходният протокол за разкомплектуване съдържа списък с настроените по рецепта компоненти на комплекта.  

![](914-assembling12.png){ class=align-center w=15cm }

5) **Заприходяване на съставни части** - Разкомплектуването се финализира с валидиране и на приходния протокол от бутон [**Приключен**] в лентата с инструменти. 

Обзавеждат се **Цена** за настроения като основен елемент и **Стойност** на документа според стойността на свързания **РПР**.  
